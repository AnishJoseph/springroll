...
import com.springroll.orm.entities.IMdmEntity;
    ...
    public void on(MdmEvent mdmEvent) {
        ...
        try {
            for (MdmChangedRecord mdmChangedRecord : mdmDTO.getChangedRecords()) {
                IMdmEntity entity = (IMdmEntity)em.find(mdmDefinition.getMasterClass(), mdmChangedRecord.getId());
                ...
                logs.addAll(mdmEvent.getReviewLog().stream().map(reviewLog -> new Reviews(reviewLog.isApproved(), reviewLog.getReviewer(), reviewLog.getReviewComment(), reviewLog.getTime())).collect(Collectors.toList()));
                entity.getReviewLogs().addAll(logs);
            }
            for (Map<String, Object> newRecord : mdmDTO.getNewRecords()) {
                ...
                IMdmEntity entity = (IMdmEntity)wrapper.getWrappedInstance();
                List<Reviews> logs = new ArrayList<>(mdmEvent.getReviewLog().size());
                logs.addAll(mdmEvent.getReviewLog().stream().map(reviewLog -> new Reviews(reviewLog.isApproved(), reviewLog.getReviewer(), reviewLog.getReviewComment(), reviewLog.getTime())).collect(Collectors.toList()));

                entity.setReviewLogs(logs);
                em.persist(entity);
            }
