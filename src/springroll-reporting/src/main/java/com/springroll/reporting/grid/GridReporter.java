...
@Component public class GridReporter implements GridReportingService {
    ...
    public List<IReportParameter> getParameters(String gridName, Map<String, Object> parameters){
        ...
        for (Parameter<?> parameter : grid.getParameters()) {
            ...
            reportParameters.add(new ReportParameter(parameter.getName(), mapJavaType(parameter.getParameterType().getName()), true, true, multiSelect, true, lovs, gridParameter == null ? null : gridParameter.getSetTime(), gridParameter.getWidth() ));
        }
